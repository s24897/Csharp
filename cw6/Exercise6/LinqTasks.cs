...
namespace Exercise6
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<object> result = Emps.Join(Depts, e => e.Deptno, d => d.Deptno, (e, d) => new { Emps = e, Depts = d }).Select(s => new { s.Emps.Ename, s.Emps.Job, s.Depts.Dname });
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Select(e => new {Ename = e.Ename, Job = e.Job, HireDate = e.HireDate}).Union(Emps.Select(e => new {Ename = "Brak wartości", Job = (String) null, HireDate = (DateTime?) null } ));
            return result;
        }
        ...
        {
            int result = result = arr.GroupBy(i => i).OrderByDescending(grp => grp.Count() % 2 != 0).Select(grp => grp.Key).First(); ;
            //result=
            return result;
        }
